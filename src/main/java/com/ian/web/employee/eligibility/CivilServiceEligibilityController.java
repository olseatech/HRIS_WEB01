...
public class CivilServiceEligibilityController {
	...
	public String saveFamilyBg(
		...
		boolean isOwnRecord = actorObj != null && actorObj.getId() == civilServiceEligibility.getEmployee().getId();
	...
	@PostMapping("/deleteCivilEligibility/{id}")
	@Transactional
	public String deleteCivilEligibility(
		...
		String redirectBase = "redirect:/employee/civil-eligibility/" + employeeId + "/" + (showMode == null ? "" : showMode) + "/" + empHashCode;

		Optional<CivilServiceEligibility> recordOptional = civilServiceEligibilityRepository.findById(id);
		...
		civilServiceEligibilityRepository.delete(recordOptional.get());
